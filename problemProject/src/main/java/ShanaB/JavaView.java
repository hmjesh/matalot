...
import java.awt.*;
/**
 * This class is the polatform for everything else, the main class
 *
...
public class JavaView {
    ...
    private JTextField connectorInput;
    private JButton okButton;
    private JComboBox connectorType;
    private JButton importFromDatabaseButton;
    private JLabel dbMassege;

    private static Database db = new Database();
    private SqlConnector connector = new SqlConnector(db);
    ...
    private JavaView() {
        ...
        okButton.addActionListener(e -> {
            switch (connectorType.getSelectedIndex()) {
                case 0:
                    connector.set_ip(connectorInput.getText());
                    break;
                case 1:
                    connector.set_password(connectorInput.getText());
                    break;
                case 2:
                    connector.set_user(connectorInput.getText());
                    break;
                case 3:
                    connector.set_port(connectorInput.getText());
                    break;
                case 4:
                    connector.set_dbName(connectorInput.getText());
                    break;
                case 5:
                    connector.set_ip(connectorInput.getText());
                    break;
            }
            dbMassege.setText(connectorInput.getText()+" entered into "+connectorType.getSelectedItem().toString());
        });
        importFromDatabaseButton.addActionListener(e -> {
            connector.read();
            connector=new SqlConnector(connector);
            dbMassege.setText("doen");
        });
    }
